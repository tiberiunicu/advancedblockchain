...
contract("test composable and merkle distributor", async ([owner, user1, user2, user3]) => {
    ...
    it("should return error if user tries to claim more tokens than available", async () => {
        ...
        try {
            ...
            assert(exception.message.indexOf('Insuficient founds!') > -1, 'FAILED - users tries to claim more tokens than available');
        }

    });

    it("should return error if user tries to claim more tokens than totalSupply", async () => {

        const cmpToken = await ComposableToken.deployed();
        const merkleDistributor = await MerkleDistributor.deployed();
        await cmpToken.setMerkleDistributor(merkleDistributor.address);
        await cmpToken.mint(10);
        ...
        try {
            ...
            assert(exception.message.indexOf('amount greater than total supply') > -1, 'FAILED - users tries to claim more tokens than total supply');
        }
